...
<template>
  ...
  >
    ...
    <NuxtLink
      :to="`/garments/${itemData.product.gender}/itemDetails/${itemData.id}`"
      draggable="false"
    ...
    >
      ...
      <div class="flex flex-col lg:gap-2">
        ...
        <p class="lg:text-xl font-semibold">
          ${{ itemData.product.price.toFixed(2) }}
        </p>
      </div>
      <div class="flex flex-col lg:gap-2">
        ...
        >
          <button
            @click="cartStore.chageQuantity(itemData.id!, itemData.size, -1)"
        ...
        <div class="hidden lg:flex items-end gap-2 mt-auto w-[10rem]">
          ...
          <p class="lg:text-2xl font-semibold">
            ${{
              (
                Number(itemData.product.price.toFixed(2)) * itemData.quantity
              ).toFixed(2)
            }}
          </p>
        </div>
