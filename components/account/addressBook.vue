...
<template>
  <div
    class="w-full min-w-[18rem] max-w-[18rem] sm:max-w-[35rem] md:max-w-full h-auto md:h-[30rem] gap-2 sm:gap-4 flex flex-col bg-white border-2 border-[#ccc] rounded-lg overflow-y-auto"
  >
    <div class="w-full flex items-center justify-between bg-[#445388] p-4">
      <span class="flex items-center gap-2">
        ...
        <p v-if="addresses" class="text-xs font-semibold text-light">
          {{ addresses.length }}/10
        </p>
        <i v-else class="pi pi-spinner pi-spin text-light"></i>
      </span>
